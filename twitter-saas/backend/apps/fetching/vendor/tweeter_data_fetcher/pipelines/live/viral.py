...
from tweeter_data_fetcher.paths import HISTORICAL_LIVE_DIR, PROJECT_ROOT
...
class ViralDetector:
    ...
    def __init__(self, config_path: Optional[str] = None, storage: Optional[LiveStorageManager] = None):
        self.project_root = PROJECT_ROOT
        self.config = self._load_config(resolve_config_path(config_path, project_root=self.project_root))
    ...
    def calculate_velocity(self, snapshots: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for metric in ["likes", "retweets", "replies", "views", "bookmarks", "quotes"]:
            velocity[f"{metric}_per_min"] = (
                self._num(snapshots[-1].get(metric)) - self._num(snapshots[0].get(metric))
            ) / minutes
        return velocity
    ...
    def calculate_multi_window_velocity(self, snapshots: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        for snapshot in snapshots:
            try:
                parsed.append((datetime.fromisoformat(str(snapshot["timestamp"])), snapshot))
            except (KeyError, TypeError, ValueError):
                continue
        if len(parsed) < 2:
            return None
        ...
        for window in [5, 30, 120]:
            start = next(
                (
                    snapshot
                    for timestamp, snapshot in reversed(parsed)
                    if (latest_ts - timestamp).total_seconds() / 60.0 >= window
                ),
                None,
            )
            if not start:
            ...
            for metric in ["likes", "retweets", "replies", "views", "quotes", "bookmarks"]:
                result[f"{metric}_per_min_{window}m"] = (
                    self._num(latest.get(metric)) - self._num(start.get(metric))
                ) / float(window)
    ...
    def calculate_acceleration(self, snapshots: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        ...
        first = self.calculate_velocity(snapshots[: midpoint + 1])
        second = self.calculate_velocity(snapshots[midpoint:])
    ...
    def calculate_momentum(self, snapshots: List[Dict[str, Any]]) -> float:
        if len(snapshots) < 4:
            return 0.0
        likes = [self._num(snapshot.get("likes")) for snapshot in snapshots[-5:]]
        differences = [likes[index] - likes[index - 1] for index in range(1, len(likes))]
        return differences[-1] - differences[0] if len(differences) >= 2 else 0.0
    ...
    def get_account_baseline(self, account: str) -> Dict[str, float]:
        ...
        for folder in ["1_user_tweets", "2_user_tweets_and_replies", "4_union"]:
            path = HISTORICAL_LIVE_DIR / "processed" / folder / key / f"{folder}.json"
            try:
                tweets = json.loads(path.read_text(encoding="utf-8")) if path.exists() else []
            except (OSError, ValueError):
                tweets = []
        ...
        for metric, metric_values in values.items():
            ordered = sorted(value for value in metric_values if value > 0)
            if ordered:
                index = min(len(ordered) - 1, int(len(ordered) * self.threshold_percentile / 100.0))
                baseline[f"{metric}_p{self.threshold_percentile}"] = ordered[index]
