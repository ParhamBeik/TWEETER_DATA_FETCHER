#!/usr/bin/env python3
"""Automated Twitter/X session refresh via Playwright.
...
Run:
    ...
    python -m tweeter_data_fetcher.x_api.auth --interactive

Flags:
    --interactive   open a headed browser for manual login
    --headed        alias for --interactive
    --config <path> config.json to refresh (else canonical config/)
"""
...
from tweeter_data_fetcher.paths import HISTORICAL_LIVE_DIR
...
def run_interactive_cli() -> None:
    ...
    try:
        ...
        print(f"\nError: {e}")
        print("Run tdf-auth --interactive after creating config/config.json.")
        sys.exit(1)
...
def main() -> None:
    ...
    configure_logging(
        subsystem="auth",
        logs_dir=HISTORICAL_LIVE_DIR / "logs",
    )
