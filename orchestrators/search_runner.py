...
class SearchTimelineMonitor:
    ...
    def _request_page(self, search_def: Dict[str, Any], raw_query: str, product: str, search_url: str, cursor: Optional[str], retries: int) -> Optional[Dict[str, Any]]:
        ...
        for attempt in range(max(1, retries)):
            response = self.api_manager.perform_get(endpoint=endpoint, url=url, max_retries=1, headers={"referer": search_url, "x-twitter-active-user": "yes"})
            if response.status_code == 200:
                return response.json()
            if response.status_code == 429:
                time.sleep(max(1, self.api_manager.rate_limit_sleep_seconds(endpoint, response.headers)))
                continue
            if response.status_code == 404 and cursor:
                return {"_failure": "mid_pagination_404", "_status": 404}
    ...
    def monitor_search(self, search_def: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for page in range(1, int(policy["pagination_safety_cap_pages"]) + 1):
            payload = self._request_page(search_def, raw_query, product, search_url, cursor, int(policy["max_retries"]))
            if not payload:
                exhausted_reason = self.classify_search_failure(self.api_manager.get_last_status("SearchTimeline"), page, cursor)
                break
