...
def extract_query_ids_from_har(har_file_path):
    ...
    try:
        ...
        for entry in har_data.get('log', {}).get('entries', []):
            ...
            if 'graphql' in url and 'x.com/i/api/graphql' in url:
                # Extract query ID and endpoint name
                # Format: https://x.com/i/api/graphql/{QUERY_ID}/{ENDPOINT_NAME}
                parts = url.split('/graphql/')
                if len(parts) > 1:
                    remaining = parts[1].split('?')[0]  # Remove query params
                    path_parts = remaining.split('/')
                    if len(path_parts) >= 2:
                        query_id = path_parts[0]
                        endpoint = path_parts[1]
                        
                        if endpoint == 'UserByScreenName':
                            query_ids['user_by_screen_name_query_id'] = query_id
                        elif endpoint == 'UserTweets':
                            query_ids['user_tweets_query_id'] = query_id
                        elif endpoint == 'UserTweetsAndReplies':
                            query_ids['user_tweets_and_replies_query_id'] = query_id
                        elif endpoint == 'TweetDetail':
                            query_ids['tweet_detail_query_id'] = query_id
...
def extract_query_ids_from_text(text):
    ...
    # Pattern: https://x.com/i/api/graphql/{QUERY_ID}/{ENDPOINT}
    pattern = r'https://x\.com/i/api/graphql/([a-zA-Z0-9_-]+)/(UserByScreenName|UserTweets|UserTweetsAndReplies|TweetDetail)'
    
    matches = re.findall(pattern, text)
    
    for query_id, endpoint in matches:
        if endpoint == 'UserByScreenName':
            query_ids['user_by_screen_name_query_id'] = query_id
        elif endpoint == 'UserTweets':
            query_ids['user_tweets_query_id'] = query_id
...
def setup_cookies():
    ...
    print("Next steps:")
    print("=" * 70)
    print("  1. Run: python3 fetch_historical_tweets.py")
...
def update_cookies_only(config, config_file):
    ...
    print("\n✅ Ready to run fetch_historical_tweets.py")
