...
NO production code is modified. Output: diagnostics/reports/dynamic_txid_<ts>.{json,md}.

Run:
    python twitter_fetcher/diagnostics/probe_dynamic_txid.py
    python twitter_fetcher/diagnostics/probe_dynamic_txid.py --accounts elonmusk Reuters \\
        --pages 8 --queries OpenAI Bitcoin
    python twitter_fetcher/diagnostics/probe_dynamic_txid.py --endpoints SearchTimeline \\
...
def main() -> int:
    ...
    cfg_path = Path(args.config) if args.config else resolve_config_path(project_root=PROJECT_ROOT)
