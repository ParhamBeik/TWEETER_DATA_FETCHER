...
"""
Save raw pages, processed tweet sets, reports, and state files behind StorageManager.
...
class StorageManager:
    ...
    SET_FOLDER_MAP: Dict[str, str] = {
        ...
        "UNION": "4_union",
        "A_MINUS_B": "5_a_minus_b",
        "REPLIES_ONLY": "6_b_minus_a",
        "B_MINUS_A": "6_b_minus_a",
        "SYMMETRIC_DIFFERENCE": "7_symmetric_difference",
        "1_user_tweets": "1_user_tweets",
        "2_user_tweets_and_replies": "2_user_tweets_and_replies",
        "3_intersection": "3_intersection",
        "4_union": "4_union",
        "5_replies_only": "6_b_minus_a",
        "5_a_minus_b": "5_a_minus_b",
        "6_b_minus_a": "6_b_minus_a",
        "7_symmetric_difference": "7_symmetric_difference",
        "1_user_tweets (A)": "1_user_tweets",
        "2_user_tweets_and_replies (B)": "2_user_tweets_and_replies",
        "3_intersection (A ∩ B)": "3_intersection",
        "4_union (A ∪ B)": "4_union",
        "5_a_minus_b (A - B)": "5_a_minus_b",
        "6_b_minus_a (B - A)": "6_b_minus_a",
        "7_symmetric_difference (A △ B)": "7_symmetric_difference",
    }
    ...
    def __init__(
        ...
        self.project_root = project_root or base_dir or Path(__file__).resolve().parents[3]
        ...
        self.a_minus_b_dir = self.processed_root / "5_a_minus_b"
        self.endpoint_diffs_dir = self.processed_root / "6_b_minus_a"
        self.symmetric_difference_dir = self.processed_root / "7_symmetric_difference"
    ...
    def get_fetch_watermark(self, username: str, endpoint: str) -> Optional[str]:
        """ISO-8601 timestamp of the last SUCCESSFUL fetch for this account+endpoint.

        Drives the unified rolling window: the next run backfills past the floored
        watermark so no partial day/hour is ever skipped. Returns None before the
        first successful fetch (caller then uses the plain configured window)."""
        ep_state = self.get_endpoint_state(username, endpoint)
        value = ep_state.get("fetch_watermark")
        return str(value) if value else None

    def set_fetch_watermark(self, username: str, endpoint: str, iso_timestamp: str) -> Path:
        """Persist the fetch watermark. Only call on a SUCCESSFUL endpoint fetch, so a
        failed/partial run never advances the watermark past unfetched data."""
