...
class APIManager:
    ...
    def _load_query_id_state(self) -> Dict[str, Dict[str, Any]]:
        state_file = self.state_dir / "query_id_state.json"
        if state_file.exists():
            try:
                with open(state_file) as f:
                    data = json.load(f)
    ...
    def _save_query_id_state(self):
        ...
        with open(state_file, "w") as f:
            json.dump(self.query_id_state, f, indent=2)
    ...
    def _mark_tx_id(self, endpoint: str, tx_id: str, state: str):
        ...
        if state == "healthy":
            self.tx_id_state[endpoint][tx_id] = {"status": "healthy", "failures": 0}
        else:
            failures = self._param_failures(self.tx_id_state[endpoint].get(tx_id)) + 1
            self.tx_id_state[endpoint][tx_id] = {
                "status": "stale" if failures >= self.PARAM_FAILURE_LIMIT else "suspect",
                "failures": failures,
            }
        self._save_tx_id_state()

    def _mark_query_id(self, endpoint: str, query_id: str, state: str):
        if endpoint not in self.query_id_state:
            self.query_id_state[endpoint] = {}
        if state == "healthy":
            self.query_id_state[endpoint][query_id] = {"status": "healthy", "failures": 0}
        else:
            failures = self._param_failures(self.query_id_state[endpoint].get(query_id)) + 1
            self.query_id_state[endpoint][query_id] = {
                "status": "stale" if failures >= self.PARAM_FAILURE_LIMIT else "suspect",
                "failures": failures,
            }
