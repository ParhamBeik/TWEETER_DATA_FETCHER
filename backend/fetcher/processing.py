#!/usr/bin/env python3
"""Tweet parsing, rolling-window coverage, and X GraphQL request contracts.

Dates are Tehran-local so day/hour boundaries line up with the rolling window
...
class TweetSetProcessor:
    """Parse raw GraphQL pages into normalized tweet dicts keyed by tweet id."""
...
def tehran_date(dt: datetime) -> str:
    """Tehran-local calendar date, the unit the rolling window counts in."""
    return _to_tehran(dt).strftime("%Y-%m-%d")


def _to_tehran(dt: datetime) -> datetime:
    return (dt.replace(tzinfo=ZoneInfo("UTC")) if dt.tzinfo is None else dt).astimezone(TZ)


def target_dates(days: int, now_dt: Optional[datetime] = None) -> List[str]:
    current = now_dt or tehran_now()
    return [tehran_date(current - timedelta(days=offset)) for offset in range(max(1, int(days)))]
...
def parse_twitter_timestamp(value: Any) -> Optional[datetime]:
    ...
    if not raw:
        return None
    try:
        return datetime.strptime(raw, "%a %b %d %H:%M:%S %z %Y").astimezone(TZ)
    except Exception:
        return None


def tweet_date(tweet: Dict[str, Any]) -> Optional[str]:
    for key in ("raw_timestamp", "created_at"):
        parsed = parse_twitter_timestamp(tweet.get(key) if isinstance(tweet, dict) else None)
        if parsed:
            return tehran_date(parsed)
...
class RollingWindowEvaluator:
    """Evaluate whether raw pages prove a Tehran rolling window."""
    ...
    def evaluate_tweets(
        ...
        targets = target_dates(window_days, now_dt=now_dt)
        if not targets:
            targets = target_dates(1, now_dt=now_dt)
        window_start = targets[-1]
        dates: Set[str] = set()
        item_count = 0
        for tweet in tweets:
            item_count += 1
            date_value = tweet_date(tweet)
            if date_value:
    ...
    def evaluate_tweets_cutoff(
        ...
        oldest_s = tehran_date(oldest_dt) if oldest_dt else None
        newest_s = tehran_date(newest_dt) if newest_dt else None
        cutoff_s = tehran_date(cutoff)
