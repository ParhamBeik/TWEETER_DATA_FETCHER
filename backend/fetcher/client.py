...
class APIManager:
    ...
    def __init__(
        ...
        # Browser-confirmed endpoint transaction IDs stay ephemeral and pinned
        # for this process. Persisting them would turn short-lived request state
        # into configuration, while rotating back to an older pool recreates 404s.
        self.browser_tx_ids: Dict[str, str] = {}
    ...
    def _generate_transaction_id(self, endpoint: str = "_default") -> str:
        ...
        """
        browser_tx_id = getattr(self, "browser_tx_ids", {}).get(endpoint)
        if browser_tx_id:
            return browser_tx_id
    ...
    def reset_transport_session(self, endpoint: Optional[str] = None, *, reason: str = "rotation") -> None:
        """Drop pooled transport state while retaining ephemeral browser IDs."""
        self.refresh_session()
        if endpoint:
            self._info(f"Reset HTTP session for {endpoint} ({reason})")
        else:
            self._info(f"Reset HTTP session ({reason})")
    ...
    def apply_browser_context(
        ...
        *,
        ...
        for endpoint, headers in (request_headers or {}).items():
            tx_id = headers.get("x-client-transaction-id")
            if tx_id:
                self.browser_tx_ids[endpoint] = str(tx_id)
    ...
    def _process_response_status(
        ...
        if response.status_code == 200:
            ...
            self.endpoint_health[endpoint] = EndpointHealth.CONTRACT_REJECTED
            self._save_endpoint_health()
        elif response.status_code in {401, 403}:
            self.endpoint_health[endpoint] = EndpointHealth.AUTH_REQUIRED
            self._save_endpoint_health()
        elif response.status_code == 404:
            # A 404 can be a cursor gate or route/session soft-block. It is not
            # evidence that a query or transaction ID is stale; only a fresh
            # browser capture can establish and promote a replacement.
            self.consecutive_404s[endpoint] = self.consecutive_404s.get(endpoint, 0) + 1
            self.endpoint_health[endpoint] = EndpointHealth.CONTEXT_REJECTED
            self._save_endpoint_health()
