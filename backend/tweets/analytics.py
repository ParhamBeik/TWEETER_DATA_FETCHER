...
from tweets import stats, topics
...
from tweets.windows import Window, accounts_from, window_from
...
class VelocityView(APIView):
    ...
    def get(self, request):
        ...
        rows, series = stats.velocity_rankings(window, handles)
...
class NarrativesView(APIView):
    ...
    def get(self, request):
        ...
        candidate_cap = _int_param(
            request, "candidates", stats.NARRATIVE_CANDIDATE_CAP, 100, 5000
        )
        if connection.vendor != "postgresql":
            return Response({"results": []})
        handles = accounts_from(request)
        try:
            rows = stats.narrative_pairs(
                window,
                handles,
